...
class Log implements LogHandlerInterface
{
    ...
    public function __construct(App $app, $config = []) {
        ...
        if (empty($this->config['format'])) {
            $this->config['format'] = '[%s][%s][%s][%s][%s] %s';
        }
        ...
        foreach ($log as $type => $val) {
            ...
            foreach ($val as $msg) {
                ...
                $message[] = $this->config['json'] ?
                    json_encode(['time' => $time, 'type' => $type, 'trace_id' => $traceId, 'runtime' => $runtime . 's', 'mem' => $mem . 'kb', 'msg' => $msg], $this->config['json_options']) :
                    sprintf($this->config['format'], $time, $type, 'trace_id:' . $traceId, 'runtime:' . $runtime . 's', 'mem:' . $mem . 'kb', $msg);
            }
